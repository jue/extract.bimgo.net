$(document).ready(function() {
  ...
  $('#projectProgressDialog').on('shown.bs.modal', function() {
    $("#projectProgressDialog").draggable({
      handle: ".modal-header"
    });
  });
...
function createProjectVignette(identifier, data) {
  ...
  if (data.progress != 'complete' && data.progress != 'failed') {
    ...
    setTimeout(function() {
      projectProgress(identifier);
    }, 5000);
  }
  if (data.progress == 'complete')
    //$('#vignette-' + identifier + ' div a.info').unbind('click').text('生成离线').attr('href', '/explore/' + identifier);
    $('#vignette-' + identifier + ' div a.info').unbind('click').text('生成离线').addClass('J_download');
}
...
var fileUploadItem = {
  ...
  /*static */
  createElt: function(identifier, filename) {
  ...
  /*static*/
  progress: function(identifier, progress) {
    $('#flow-file-' + identifier + ' div.flow-file-progress progress')
      .prop('value', progress);
  },

  /*static*/
  success: function(identifier, message) {
  ...
  /*static*/
  error: function(identifier, message) {
    $('#flow-file-' + identifier)
      .removeClass('alert-info')
...
function extractionProgress(root) {
  $.ajax({
    ...
    if(results._progress < 100){
      console.log('已下载'+ results._progress+'%')
      setTimeout(function() { extractionProgress(root); }, 1000);
    }else{
      console.log(root+' ||下载已完成'+ results._progress+'%');
      $('.J_svfTitle').text($('#vignette-'+root).attr('title'));
      $('.J_svfUrl').val(root+'/Resource/____/3D/3D.svf');
      $('#showModelUrl').modal();
    }
  })
    .fail(function(error) {
...
$(function() {
  $('body').on('click', '.J_download', function() {
    var $root = $(this).attr('target');
    $.ajax({
      url: '/api/results/' + $root + '/project',
      type: 'get',
      data: { email: '', sqllite: false, viewer_files: false },
      complete: null
    }).done(function(results) {
      setTimeout(function() { extractionProgress($root); }, 1000);
    }).fail(function(error) {
